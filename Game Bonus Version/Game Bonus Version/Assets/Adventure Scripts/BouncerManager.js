...
var templeState : int; // 0, no entry, 1 can enter, 2, passage open
...
function LoadState (newState : int) {
   ...
   if(newState > 0) {
      if(gameObject.Find("Temple Blocker"))gameObject.Find("Temple Blocker").active = false; //disable the blocker 
      gameObject.Find("FX Range").SendMessage("KillEffect");
      gameObject.Find("FX Range").active = false; //disable the fx checker
   }
   else return; 
